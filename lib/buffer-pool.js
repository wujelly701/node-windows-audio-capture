...
class BufferPool {
   ...
   * 获取统计信息
   * 
   * @returns {Object} 统计对象，包含原始数值和计算后的比率
   */
  getStats() {
    const hitRate = this.stats.allocated > 0
      ? (this.stats.hits / this.stats.allocated)
      : 0;
    
    const reuseRate = this.stats.allocated > 0
      ? (this.stats.reused / this.stats.allocated)
      : 0;
    ...
    return {
      ...
      hitRate,           // 原始比率 (0-1)
      reuseRate,         // 原始比率 (0-1)
      totalAcquires: this.stats.allocated,
      totalReleases: this.stats.released,
      totalMemory: this._getTotalMemory(),
   ...
   */
  getMemoryUsage() {
    return (this._getTotalMemory() / 1024 / 1024).toFixed(2);
  }
}
