 ...
 * - BufferPool integration for reduced memory allocation and GC pressure
 * 
...
class AudioResampler {
  ...
  constructor(options = {}) {
    ...
    // Initialize BufferPool (singleton, shared across all instances)
    this.bufferPool = BufferPool.default;
    
    // Warmup buffer pool with typical sizes for this configuration
    // Calculate typical buffer sizes based on sample rate and channels
    const typicalInputSize = Math.ceil(1024 * this.channels * 4); // ~1024 samples
    const typicalOutputSize = Math.ceil(typicalInputSize / this.ratio);
    this.bufferPool.warmup(5); // Pre-allocate 5 buffers per size
   ...
   * Call this method when you're done with a buffer returned by resample()
   * 
   * @param {Buffer} buffer - Buffer to release
   * @example
   * const resampled = resampler.resample(inputBuffer);
   * // ... use resampled buffer ...
   * resampler.releaseBuffer(resampled);
   */
  releaseBuffer(buffer) {
    if (buffer && Buffer.isBuffer(buffer)) {
      // Get the original buffer (before slice)
      const originalBuffer = buffer.parent || buffer;
      this.bufferPool.release(originalBuffer);
    }
  }
   ...
   * Uses BufferPool for efficient memory management
   * @private
   */
  _writeSamples(samples) {
    const bytesPerSample = this.outputFormat === 'float32' ? 4 : 2;
    const bufferSize = samples.length * bytesPerSample;
...
if (require.main === module) {
  ...
  qualities.forEach(quality => {
    ...
    // Clean up: release buffer back to pool
    setTimeout(() => {
      resampler.releaseBuffer(outputBuffer);
      console.log('  [BufferPool] Buffer released back to pool');
    }, 100);
    ...
    // Show BufferPool stats
    const poolStats = resampler.bufferPool.getStats();
