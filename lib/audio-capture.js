...
class AudioCapture extends Readable {
  ...
  /**
   * v2.9.0: Get all audio input (capture) devices - MICROPHONES
   * @returns {Promise<AudioDeviceInfo[]>} Array of available microphone devices
   * @since v2.9.0
   */
  static async getAudioInputDevices() {
    try {
      // Check if the native method exists
      if (!addon.getAudioInputDevices) {
        throw new Error('getAudioInputDevices is not available in this version');
      }

      // Call native module to get input device information
      const devices = addon.getAudioInputDevices();
      
      // Validate return value
      if (!Array.isArray(devices)) {
        throw new TypeError('Native getAudioInputDevices must return an array');
      }
      ...
      return devices.map((device, index) => {
        ...
        return {
          ...
          isDefault: device.isDefault,
          isActive: device.isActive,
          type: 'capture'  // Mark as capture device
        };
      });
    } catch (error) {
      throw new Error(`Failed to get audio input devices: ${error.message}`);
    }
  }

  /**
   * v2.9.0: Get the default audio input (capture) device ID - DEFAULT MICROPHONE
   * @returns {Promise<string|null>} Default microphone device ID or null if not found
   * @since v2.9.0
   */
  static async getDefaultInputDeviceId() {
    try {
      // Check if the native method exists
      if (!addon.getDefaultInputDeviceId) {
        throw new Error('getDefaultInputDeviceId is not available in this version');
      }

      const deviceId = addon.getDefaultInputDeviceId();
      return deviceId || null;
    } catch (error) {
      throw new Error(`Failed to get default input device ID: ${error.message}`);
    }
  }
