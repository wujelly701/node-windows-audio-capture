...
**版本历史**:
- [v2.9 Release Notes](docs/V2.9_RELEASE_NOTES.md) - 麦克风捕获 + ASR 质量提升
...
**音频质量增强版** - AGC 自动增益控制 + 3-Band 均衡器，< 5ms 超低延迟！

- [x] **自动增益控制（AGC）** - 音量自动归一化 🎚️
- [x] **3-Band 均衡器（EQ）** - 低/中/高频独立调节（-20~+20dB）
- [x] **超低延迟处理** - < 5ms AGC + EQ 处理延迟 ⚡
- [x] **即插即用设计** - 简单参数即可启用
- [x] **ASR 质量提升** - 稳定音量提升识别准确率
- [x] **完整文档体系** - 详细 API 文档和示例
- [x] **100% 向后兼容** - opt-in 设计，零风险
...
[📖 查看完整 v2.8.0 发布说明 →](docs/V2.8_RELEASE_NOTES.md)

### v2.9.0 完成 ✅ 🎙️

**麦克风捕获 + ASR 质量提升** - 完整音频输入解决方案！

- [x] **MicrophoneCapture API** - 麦克风直接捕获（WASAPI 直接模式）🎙️
- [x] **设备选择支持** - `deviceId` 参数选择特定麦克风
- [x] **音频效果集成** - RNNoise + AGC + EQ 完整支持
- [x] **ASR 重采样升级** - Linear → Sinc（Kaiser 窗函数）插值 🔊
- [x] **音质显著提升** - 48kHz→16kHz 降采样质量大幅改善
- [x] **低性能开销** - < 0.2ms 额外延迟，可忽略
- [x] **完整文档体系** - API 文档 + 快速入门示例
- [x] **100% 向后兼容** - 零迁移成本

**成果**：
- Sinc 性能: 180x 实时率（20s → 111ms）
- 初始化时间: 4ms（一次性）
- 音质提升: ⭐⭐⭐ → ⭐⭐⭐⭐⭐（ASR 场景）

[📖 查看完整 v2.9.0 发布说明 →](docs/V2.9_RELEASE_NOTES.md)
...
- [ ] **Zero-Copy 默认启用**（v2.6/v2.7/v2.8/v2.9 验证完成）⭐⭐ 高优先级
